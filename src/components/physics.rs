...
pub fn decelerate1d(speed: f32, decel: f32, time_step: f32) -> f32 {
    let original_sign = if speed > 0.0 { 1.0 } else if speed < 0.0 { -1.0 } else { 0.0 };
...
impl BoundingBox2D {
    ...
    pub fn translate(&self, point: Point) -> BoundingBox2D {
        BoundingBox2D{
            corners: [[self.corners[0][0] + point[0], self.corners[0][1] + point[1]],
    ...
    pub fn intersects(&self, bbox: &BoundingBox2D) -> bool {
        bbox.contains(self.corners[0]) ||
            ...
            bbox.intersects(self)
    }

    pub fn contains(&self, point: Point) -> bool {
        point[0] >= self.corners[0][0] && point[0] <= self.corners[1][0] &&
        point[1] >= self.corners[0][1] && point[1] <= self.corners[1][1]
    }
}
